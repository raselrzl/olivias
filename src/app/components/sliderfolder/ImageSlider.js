...
export default function ImageSlider() {
  ...
  <div className='absolute bottom-4 w-full flex justify-center py-2'>
    {slides.map((slide, slideIndex) => (
      ...
      >
        <p className={`w-4 h-4 md:w-6 md:h-6 lg:w-8 lg:h-8 flex items-center justify-center text-white bg-black mx-1 mb-4 cursor-pointer hover:bg-gray transition ${slideIndex === currentIndex ? 'bg-gray-500' : ''}`}>
          {slideIndex + 1}
        </p>
      </div>
    ))}
